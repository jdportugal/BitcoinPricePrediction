...
def build_timeseries(mat, y_col_index):
    ...
    y = np.zeros((dim_0,5))
    for i in range(dim_0-5):
        x[i] = mat[i:TIME_STEPS+i]
        y[i] = mat[TIME_STEPS+i:TIME_STEPS+i+5, y_col_index]
        print(y[i])
...
lstm_model.add(LSTM(250, batch_input_shape=(BATCH_SIZE, TIME_STEPS, x_t.shape[2]), dropout=0.0, recurrent_dropout=0.0, stateful=False,   kernel_initializer='random_uniform'))
...
lstm_model.save('5stepsNet.h5') 

lstm_model = load_model('10stepsNet.h5') 
gotten = lstm_model.predict(x_test_t[-210:-10],batch_size=BATCH_SIZE)
print("##########")
print(len(gotten))
print("##########")
...
print("##########")
print("##########")
y_test_t = y_test_t[-200:,0]
real =min_max_scaler_y.inverse_transform(y_test_t.reshape(-1,1))
print(len(real))
plt.figure()

plt.plot(gotten[:,0], color="g", label="gotten")
